...
public class LaunchItem extends JobBase {
      ...
      wrapper.set(new ActionComposerBuilder()
          .prepareActionSequence()
            ...
            .waitUntil(ExpectedConditions.or(
                ...
                ExpectedConditions.and(
                  ExpectedConditions.elementToBeClickable(By.id("userid")),
                  ExpectedConditions.elementToBeClickable(By.id("userpass")),
                  ExpectedConditions.elementToBeClickable(By.id("btnLogin")))), 15000)
            .custom(ac -> {
              WebDriver driver = ac.getWebDriver();
              WebElement nicknameE = driver.findElements(By.id("header_user_nick")).stream()
              ...
              if (nicknameE != null && nicknameE.isDisplayed()) {
                ...
                if (!nameOnPage.isEmpty()) {
                  if (account.equals(nameOnPage)) {
                    ...
                    new Actions(driver).moveToElement(nicknameE).moveToElement(
                        driver.findElement(By.id("header_logout_link"))).click().build().perform();
                    try {
            ...
            .custom(ac -> {
              ...
              WebElement smsBtnE = driver.findElements(By.id("btn_send_sms")).stream()
                  .findFirst().orElse(null);
          ...
          .onFail(ac -> {
            ...
            if (wrapper.get().getLoginResultStatus() == null
                || LoginTaskContext.LoginResultStatus.WaitingToLogin.equals(
                    wrapper.get().getLoginResultStatus())) {
              wrapper.get().setLoginResultStatus(LoginTaskContext.LoginResultStatus.UnknownFail);
            }
          })
          .onDone(ac -> {
  ...
  private BasicActionComposer getLaunchItemBrowserTask() {
    ...
    BasicActionComposer task = new ActionComposerBuilder()
        .prepareActionSequence()
          .getUrl("https://mybidu.ruten.com.tw/upload/item_initial.php")
          .waitUntil(
            ExpectedConditions.and(
              ExpectedConditions.visibilityOfElementLocated(By.className("class-path-node-list")),
              ExpectedConditions.presenceOfElementLocated(By.id("shop_id")),
              ExpectedConditions.visibilityOfElementLocated(
                  By.xpath("//*[@data-ref='addLocalFakeButton']")),
              ExpectedConditions.presenceOfElementLocated(
                  By.xpath("//*[@data-ref='addLocalInput']"))), 10000)
          .justWait(1000)
          .custom(ac -> {
            WebDriver driver = ac.getWebDriver();
            //category
          ...
          .waitUntil(ExpectedConditions.numberOfElementsToBe(
            By.xpath(
              ...
              + "*[@class='img' and starts-with(@style,'background-image: url(')]"), 3), 30000)
          .prepareSendKey(
              By.id("g_name"), "PS3用ソフト　METAL GEAR SOLID 4 GUNS OF THE PATRIOTS"
                  + "　メタルギアソリッド4 ガンズ・オブ・ザ・パトリオット").withClearBeforeSend().done()
          .prepareSendKey(By.name("g_direct_price"), "99999").withClearBeforeSend().done()
          .prepareSendKey(By.id("show_num"), "1").withClearBeforeSend().done()
          .selectByText(By.id("location_tw"), "台北市")
          .click(By.id("cb_dway_self"))
          .click(By.cssSelector("#main_form input[type='submit']"))
